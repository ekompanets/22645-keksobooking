...
(function () {
  ...
  var getAddress = function (posX, posY) {
    return {
      x: Math.floor(posX + adData.PIN_MAIN_WIDTH / 2) ,
      y: (posY + adData.PIN_MAIN_HEIGHT)
    }
  };
  ...
  window.dragNDrop.setDragable(pinMain, pinMain, function (posX, posY) {
    ...
    var canMove = false;
    if ((adr.x >= adData.MIN_X_LOCATION) && (adr.x <= adData.MAX_X_LOCATION) && (adr.y >= adData.MIN_Y_LOCATION) && (adr.y <= adData.MAX_Y_LOCATION)) {
